...
function getAIContext() {
  ...
  return `\n\n[Contexto da Ficha Atual]\n` +
    `Personagem: ${ctx.name || 'sem nome'} | Clã: ${ctx.clan || 'não escolhido'} | ` +
    `Trilha: ${ctx.path || 'Humanidade'} | Humanidade: ${ctx.humanity} | ` +
    `FdV: ${ctx.willpower} | Pts. Sangue: ${ctx.bloodPts}` +
    (ctx.clan
...
async function sendAI() {
  ...
  try {
    ...
    // Keep history bounded to ~12 exchanges
...
function askCharAnalysis() {
  syncFormState();
  updateContext();
  const ctx = window.__charContext;
  let msg   = 'Analise minha ficha atual e me dê sugestões de melhoria.';
  if (ctx && ctx.name) msg += ` Personagem: ${ctx.name}`;
  askQuick(msg);
}
